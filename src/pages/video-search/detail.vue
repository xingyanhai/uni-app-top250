<template>
	<view class="new-page">
		...
		<view class="down-box" v-if="downList.length || resource.length">
			...
			<view class="content">
				<view v-for="(value, index) in downList" class="item primary" :data-item="value" @click="copy1" :key="index">
					点击复制下载链接{{index + 1}}
				</view>
				<view v-for="(value, index) in resource" class="item primary" :data-item="value" @click="copy2" :key="index">
					点击复制下载链接{{index + 1}}
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			onPlay () {
				...
				if (!this.isShare && this.config && this.config.showShare) {
					this.timer = setTimeout(() => {
						this.videoContext.pause()
						this.$refs.sharePopup.open()
					}, 3000)
				} else if (!this.isLookVideoAd && this.config && this.config.showAd) {
					this.timer = setTimeout(() => {
						this.videoContext.pause()
						uni.showModal({
							title: '提示',
